...
contract PlayCoinMultiSigWallet {
  address owner;
  address[] signers;
  mapping (address => bool) signerMap; // instead of iterating array, we will use this to check signer

  uint8  requiredNoOfSign;
  uint16 nextTx;

  address public playcoinAddr;

  /* GMT Wednesday, 16 November 5138 9:46:39 AM */
  ...
  mapping (uint16 => TXRequest) requestMap;
  mapping (uint16 => address[]) requestSignMap;
  mapping (uint16 => address[]) requestRejectMap;
  ...
  /* for debugging */
  event DebugMsg0(string msg);
  event DebugMsg1(string msg, uint value1);
  event DebugMsg1Bytes(string msg, bytes value1);
  event DebugMsg1String(string msg, string s);
  event DebugMsg1A(string msg, address value1);
  event DebugMsg1Bool(string msg, bool value1);
  event DebugMsg2(string msg, uint value1, uint value2);
  event DebugMsg3(string msg, uint value1, uint value2, uint value3);
  ...
  function calcTime(uint time, uint year, uint week, uint day, uint hour, uint min)
    ...
    returns(uint)
  {
    return (time + year * (1 years) + week * (1 weeks)
    + day * (1 days) + hour * (1 hours) + min * (1 minutes));
  }
   ...
   *
   */
  function PlayCoinMultiSigWallet(address[] _signers, uint8 _requiredSign, address _playcoinAddr)
    public
  {
  ...
  function requestTX(bytes _abiData, string _description) public senderIsSigner returns(uint16) {
    requestTXWithContractAndExpiry(0, FOREVER, _abiData, _description);
  }
  ...
  /**
    ...
    */
  function requestTXWithContract(address _contractAddr, bytes _abiData, string _description) public senderIsSigner returns(uint16) {
    requestTXWithContractAndExpiry(_contractAddr, FOREVER, _abiData, _description);
  }
  ...
  /**
    ...
    */
  function requestTXWithContractAndExpiry(address _contractAddress, uint _expiry, bytes _abiData, string _description) public senderIsSigner returns(uint16) {
    require(_expiry > now + 1 days); // must provide at least 1 days expiry
    require(nextTx<60000); // max 600000 tx
    ...
    emit TXSigned(txid, msg.sender, 1);  // request a TX is also a signing that TX
  ...
  function rejectTX(uint16 txid) public senderIsSigner returns(uint8) {
    ...
    uint8 nRejected=0;  // number of person who rejected the transfer

    // check if sender is signed already
    for(i=0; i<requestSign.length; ++i) {
      require(requestSign[i] != msg.sender);  // fail if the sender already signed
    }

    // check if sender already rejected and count all person who rejected this transfer
    for(i=0; i<requestReject.length; ++i) {
      if(requestReject[i] == msg.sender) {
        require(false);  // fail if already rejected
      }
      ++nRejected;
    }
    ...
    if(nRejected > (uint8)(signers.length) - requiredNoOfSign) {
      request.cancelled = true;
      emit TXCancelled(txid);
    }
    ...
    returns(bool)
  {
    ...
    // check the existing pending TXs
    // and if there are pending TX, this user cannot be removed
    for(i=0; i<nextTx; ++i) {
      TXRequest storage request = requestMap[i];
      if(!request.cancelled && !request.confirmed) {
        emit SignerCannotRemoved(_signer, "pending TX exists.");
        return false;
      }
    }
    ...
    signers.length--; // decrease array length
  ...
  /**
   * @dev send the Qtum stored in this contract to receiver
   *
   * This should be done by using the requestTXWithContract function.
   *
   */
  function refundQtum(address receiver, uint amount) public callingSelf {
    require(address(this).balance >= amount);

    if(receiver.send(amount)) {
      emit QtumRefunded(receiver, amount);
    } else {
      emit QtumRefundFailed(receiver, amount);
    }
  }
