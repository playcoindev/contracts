...
contract PlayCoin is StandardToken {
   ...
   */
  function PlayCoin(string _symbol, string _name, address _owner, address _admin, address _vault) public {
    require(bytes(_symbol).length > 0);
    require(bytes(_name).length > 0);
    
    totalSupply_ = noOfTokens * (10 ** uint(decimals));  // 1E9 tokens initially
   ...
   *
   */
  function setReserve(address _address, uint reserve) onlyAdmin public {
    require(reserve <= totalSupply_);
    require(_address != address(0));

    reserves[_address] = reserve;
    emit ReserveChanged(_address, reserve);
  }
   ...
   */
  function setVault(address _newVault) public onlyOwnerOrVault {
   ...
   */
  function setOwner(address _newOwner) public onlyOwnerOrVault {
   ...
   */
  function recall(address _from, uint _amount) public onlyVault {
