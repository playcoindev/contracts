...
contract("PlayCoin", (accounts) => {
  ...
  it("coin initial values", async () => {
    ...
    assert.equal(await coin.totalSupply(), NoOfTokens * OnePlayCoin);  // 1B * 10 decimals
    assert.equal(await coin.balanceOf(owner), 0);  // 0
    assert.equal(await coin.balanceOf(admin), 0);  // 0
    assert.equal(await coin.balanceOf(vault), NoOfTokens * OnePlayCoin);  // 1B * 10 decimals
  ...
  /* reserve                                                                  */
  /****************************************************************************/
  it("only vault can set reserve", async () => {
  ...
  it("setOwner", async () => {    
    ...
    owner = new2;
    assert.equal(new2, await coin.getOwner({from: owner}));
    ...
    try {
      ...
      assert.fail();    // only owner or vault can set owner
    }
    catch(exception) {
      assert.isTrue(exception.message.includes("revert"));
    }
  ...
  it("setVault", async () => {    
    ...
    // check transfer
    assert.equal(bal, await coin.balanceOf(vault));      
    assert.equal(0, await coin.balanceOf(oldVault));    
        
    await coin.setVault(new1, {from: vault});
    vault = new1;
    assert.equal(new1, await coin.getVault({from: vault}));
